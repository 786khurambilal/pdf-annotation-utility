...
const CTAArea = styled.div.withConfig({
  shouldForwardProp: (prop) => !['x', 'y', 'width', 'height', 'isAutoGenerated'].includes(prop as string)
})<{
  x: number;
  y: number;
  width: number;
  ...
  pointer-events: auto;
  cursor: pointer;
  border: ${props => props.isAutoGenerated ? '1px dashed #28a745' : '2px solid #007bff'};
  background-color: ${props => props.isAutoGenerated ? 'rgba(40, 167, 69, 0.05)' : 'rgba(0, 123, 255, 0.1)'};
  border-radius: ${props => props.isAutoGenerated ? '2px' : '4px'};
  ...
  &:hover {
    background-color: ${props => props.isAutoGenerated ? 'rgba(40, 167, 69, 0.15)' : 'rgba(0, 123, 255, 0.2)'};
    border-color: ${props => props.isAutoGenerated ? '#1e7e34' : '#0056b3'};
    box-shadow: 0 2px 8px ${props => props.isAutoGenerated ? 'rgba(40, 167, 69, 0.2)' : 'rgba(0, 123, 255, 0.3)'};
  }

  &:active {
    background-color: ${props => props.isAutoGenerated ? 'rgba(40, 167, 69, 0.3)' : 'rgba(0, 123, 255, 0.3)'};
    transform: scale(0.98);
  }
`;

const CTAIcon = styled.div.withConfig({
  shouldForwardProp: (prop) => prop !== 'isAutoGenerated'
})<{ isAutoGenerated?: boolean }>`
  background-color: ${props => props.isAutoGenerated ? '#28a745' : '#007bff'};
  color: white;
  border-radius: 50%;
  width: 24px;
  height: 24px;
  ...
  ${CTAArea}:hover & {
    background-color: ${props => props.isAutoGenerated ? '#1e7e34' : '#0056b3'};
    transform: scale(1.1);
  }
`;

const Tooltip = styled.div.withConfig({
  shouldForwardProp: (prop) => !['isVisible', 'x', 'y'].includes(prop as string)
})<{
  isVisible: boolean;
  x: number;
  y: number;
    ...
    <>
      <OverlayContainer hasContextMenu={!!contextMenu} onClick={handleOverlayClick}>
        {pageCTAs.map((cta) => {
          ...
          // Debug logging for auto-generated CTAs (QR codes)
          if (cta.isAutoGenerated && process.env.NODE_ENV === 'development') {
            console.log(`🔗 Rendering CTA overlay for ${cta.label}:`, {
              original: cta.coordinates,
              scale,
              scaled: scaledCoords,
              url: cta.url
            });
          ...
          return (
            <CTAArea
              ...
              title={`${cta.label} - ${cta.url}${cta.isAutoGenerated ? ' (Auto-generated from QR code)' : ''}`}
              role="button"
              tabIndex={0}
              aria-label={`Call to action: ${cta.label}${cta.isAutoGenerated ? ' (Auto-generated from QR code)' : ''}`}
              onKeyDown={(e) => {
            ...
            >
              <CTAIcon isAutoGenerated={cta.isAutoGenerated}>
                {cta.isAutoGenerated ? '📱' : '🔗'}
              </CTAIcon>
              
              {/* Debug coordinates display for auto-generated CTAs */}
              {cta.isAutoGenerated && process.env.NODE_ENV === 'development' && (
                <div style={{
                  position: 'absolute',
                  top: '-20px',
                  left: '0',
                  ...
                  {cta.label}: {Math.round(scaledCoords.x)},{Math.round(scaledCoords.y)} ({Math.round(scaledCoords.width)}×{Math.round(scaledCoords.height)})
                </div>
              )}
            </CTAArea>
          );
        ...
        {hoveredCTA && (
          ...
          >
            {hoveredCTA.label}
            {hoveredCTA.isAutoGenerated && (
              <>
                <br />
                <small>📱 Auto-generated from QR code</small>
                {hoveredCTA.qrCodeContent && (
                  <>
                    <br />
                    <small>QR Content: "{hoveredCTA.qrCodeContent}"</small>
                  </>
                )}
              </>
            )}
