...
export class AnnotationManager {
  ...
  public createCallToAction(
    ...
    const id = this.generateId();
    const callToAction = createCallToAction(id, userId, documentId, pageNumber, url, label, coordinates, isAutoGenerated, qrCodeContent);
  ...
  public updateCallToAction(
    ...
    // If this was an auto-generated CTA and it's being edited, convert it to manual
    if (callToAction.isAutoGenerated && (updates.url || updates.label)) {
      updatedCallToAction.isAutoGenerated = false;
      // Remove auto-generation metadata when converting to manual
      delete updatedCallToAction.qrCodeContent;
      delete updatedCallToAction.autoGeneratedAt;
    }
  ...
  /**
   * Check if an auto-generated CTA already exists for the given QR code content
   */
  public hasAutoGeneratedCTA(userId: string, documentId: string, qrCodeContent: string, pageNumber?: number): boolean {
    if (!userId?.trim() || !documentId?.trim() || !qrCodeContent?.trim()) {
      return false;
    }

    try {
      const annotations = this.getAnnotations(userId, documentId);
      if (!annotations || !Array.isArray(annotations.callToActions)) {
        return false;
      }
      
      return annotations.callToActions.some(cta => 
        cta.isAutoGenerated && 
        cta.qrCodeContent === qrCodeContent &&
        (pageNumber === undefined || cta.pageNumber === pageNumber)
      );
    } catch (error) {
      return false;
    }
  ...
  /**
   * Get existing auto-generated CTA for the given QR code content
   */
  public getAutoGeneratedCTA(userId: string, documentId: string, qrCodeContent: string, pageNumber?: number): CallToAction | null {
    if (!userId?.trim() || !documentId?.trim() || !qrCodeContent?.trim()) {
      return null;
    }
    ...
    try {
      ...
      return annotations.callToActions.find(cta => 
        cta.isAutoGenerated && 
        cta.qrCodeContent === qrCodeContent &&
        (pageNumber === undefined || cta.pageNumber === pageNumber)
      ) || null;
